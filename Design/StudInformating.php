...
// '$MotherName', '$MotherReligion', '$MotherLocale', '$MotherAddress', '$MotherContactNo', NULL, '$MotherOccupation', '$MotherWorkAddress', '$MotherWorkContactNo')
    ...
    $statement = $connection->prepare("UPDATE STUDENTINFO SET FROMYEAR=?, TOYEAR=?, GRADELEVEL=?, LASTNAME=?, FIRSTNAME=?, MIDDLENAME=?, BIRTHDATE=?, BIRTHMUNTOWCITY=?, BIRTHPROVINCE=?, GENDER=?, LASTSCHOOLATTENDED=?, LASTSCHOOLYEAR=?, LASTSCHOOLGRADELEVEL=?,LASTSCHOOLADDRESS=? WHERE ID = $ID");

    $statement2 = $connection->prepare("UPDATE RELIGION SET RELIGIOUSAFFILIATION=?, INCAREA=?, INCGROUP=?, INCLOCALE=?, INCDISTRICT=?, INCRECOMMENDNAME=?, INCRECOMMENDADDRESS=?, INCRECOMMENDCONTACTNO=? WHERE ID=$ID");

    $statement3 = $connection->prepare("UPDATE PARENTS SET FATHERNAME=?,FATHERRELIGION=?,FATHERLOCALE=?,FATHERADDRESS=?,FATHERCONTACT=?,FATHEROCCUPATION=?,FATHERWORKADDRESS=?,FATHERWORKCONTACTNO=?,MOTHERNAME=?,MOTHERRELIGION=?,MOTHERLOCALE=?,MOTHERADDRESS=?,MOTHERCONTACT=?,MOTHEROCCUPATION=?,MOTHERWORKADDRESS=?,MOTHERWORKCONTACTNO=? WHERE ID=$ID");


// INSERT INTO CITIZENTEMP(CITIZENSHIP, VISATYPE, VISAISSUEDIN, VEXPIRATION, PASSPORTNO, PISSUEDIN, PEXPIRATION, COMPLETEADDRESS, CONTACTNO) 
//     VALUES ('FILIPINO','Visa','2019-12-02','2019-12-17','11112222','2019-12-03','2019-12-22','Caloocan','09999222212');

    $statement->execute([$fromYear, $toYear, $level, $lastName, $firstName, $MiddleName, $Birthdate, $MunTowCity, $Province, $gender, $LastSchoolname, $LastSchoolYear, $GradeLevel, $LastSchoolAddress]);
    ...
    if ($_POST['NonFil'] != "") {
    $statement1 = $connection->prepare("UPDATE CITIZEN SET CITIZENSHIP=?, VISATYPE=?, VISAISSUEDIN=?, VEXPIRATION=?, PASSPORTNO=?, PISSUEDIN=?, PEXPIRATION=?, COMPLETEADDRESS=?, CONTACTNO=? WHERE ID=$ID");

    $statement1->execute([$Citizen, $VisaType, $VisaIssuedIn, $VExpiration, $PassportNo, $PIssuedin, $PExpiration, $CompleteAddress, $ContactNo]);
    } else {   
    $statement1 = $connection->prepare("UPDATE CITIZEN SET CITIZENSHIP=?, COMPLETEADDRESS=?, CONTACTNO=? WHERE ID=$ID");

    $statement1->execute([$Citizen, $CompleteAddress, $ContactNo]); 
    }
...
header('Location: StudInformatted.php');
?>